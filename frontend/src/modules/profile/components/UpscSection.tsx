...
const EXAM_LABELS: Record<string, string> = {
  cse: 'UPSC CSE', capf: 'CAPF', cds: 'CDS',
  ies: 'IES', cms: 'CMS', state_pcs: 'State PCS', other: 'Other',
}
const STAGE_LABELS: Record<string, string> = {
  none: 'None', prelims: 'Prelims', mains: 'Mains', interview: 'Interview',
}

export function UpscSection({ profile, open, onToggle }: Props) {
  ...
  const mut = useMutation({
    mutationFn: () => onboardingApi.saveUpscJourney({
      ...form,
      upsc_exam: form.upsc_exam as any,
      highest_stage_cleared: form.highest_stage_cleared as any,
  ...
  return (
    <ProfileSection title="UPSC Journey" summary={summary} isOpen={open} onToggle={onToggle} saving={mut.isPending} saved={saved}>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(EXAM_LABELS).map(([val, label]) => (
              <button
                key={val} type="button"
                onClick={() => setForm(p => ({ ...p, upsc_exam: val }))}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="grid grid-cols-4 gap-2">
            {Object.entries(STAGE_LABELS).map(([val, label]) => (
              <button
                key={val} type="button"
                onClick={() => setForm(p => ({ ...p, highest_stage_cleared: val }))}
                className={cn('h-10 rounded-xl border text-xs font-medium transition-all',
                  form.highest_stage_cleared === val
                    ? 'bg-primary text-white border-primary'
                    : 'bg-white text-gray-600 border-gray-200 hover:border-primary/50')}
              >
                {label}
