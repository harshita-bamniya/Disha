...
export default function CandidateReportsPage() {
  ...
  return (
    <section className="flex flex-col gap-6">
      ...
      <div>
        <h1 style={{ fontSize: 20, fontWeight: 800, color: colors.text.ink, fontFamily: 'Hind, sans-serif', letterSpacing: '-0.3px' }}>Candidate Reports</h1>
        <p className="text-sm mt-1" style={{ color: colors.text.muted }}>Registration trends, application behaviour, and score distribution.</p>
      </div>
          ...
          {userGrowth.length > 0 && (
            <div style={{ background: '#fff', borderRadius: 16, border: '1px solid rgba(0,0,0,0.08)', padding: '20px' }}>
              ...
              <div className="flex items-end gap-0.5 h-28">
                {userGrowth.map(p => (
                  <div
                    ...
                    style={{ height: `${(p.count / maxGrowth) * 100}%`, minHeight: p.count > 0 ? 3 : 1, flex: 1, background: colors.brand.navy, borderRadius: 2, transition: 'opacity 0.15s' }}
              ...
              <div className="flex justify-between mt-2">
                <span className="text-[10px]" style={{ color: colors.text.muted }}>{userGrowth[0]?.date}</span>
                <span className="text-[10px]" style={{ color: colors.text.muted }}>{userGrowth[userGrowth.length - 1]?.date}</span>
              </div>
            </div>
          )}
                ...
                {scoreDist.map(bin => (
                  <div key={bin.range} className="flex-1 flex flex-col items-center gap-1.5">
                    <div
                      ...
                      style={{ height: `${(bin.count / maxScore) * 100}%`, minHeight: bin.count > 0 ? 3 : 0, width: '100%', background: colors.brand.navy, borderRadius: 2, opacity: 0.75, transition: 'opacity 0.15s' }}
                      onMouseOver={e => (e.currentTarget.style.opacity = '1')}
                      onMouseOut={e => (e.currentTarget.style.opacity = '0.75')}
                    />
                    <span className="text-[9px] text-center leading-tight" style={{ color: colors.text.muted }}>{bin.range}</span>
                  </div>
                ))}
              </div>
            </div>
