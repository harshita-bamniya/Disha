...
export default function FinancialReportsPage() {
  ...
  return (
    <section className="flex flex-col gap-6">
      ...
      <div>
        <h1 style={{ fontSize: 20, fontWeight: 800, color: colors.text.ink, fontFamily: 'Hind, sans-serif', letterSpacing: '-0.3px' }}>Financial Reports</h1>
        <p style={{ fontSize: 14, color: colors.text.muted, marginTop: 4 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 16, border: '1px solid rgba(0,0,0,0.08)', padding: 20 }}>
        ...
        {data.trend.length === 0 ? (
          <p style={{ fontSize: 12, color: colors.text.muted }}>No subscriptions in this window yet.</p>
        ) : (
          <div className="flex items-end gap-3" style={{ height: 96 }}>
            {data.trend.map(t => (
              <div key={t.month} className="flex-1 flex flex-col items-center gap-1.5">
                ...
                />
                <span style={{ fontSize: 10, color: colors.text.muted }}>{t.month.slice(5)}</span>
              </div>
            ))}
          </div>
        )}
          ...
          data.plan_distribution.map((p, idx) => (
            ...
            >
              <div className="min-w-0">
                <p style={{ fontSize: 14, fontWeight: 600, color: colors.text.ink, textTransform: 'capitalize' }}>{p.plan_name}</p>
              ...
              <div className="text-right shrink-0">
                <p style={{ fontSize: 14, fontWeight: 900, color: colors.text.ink }}>{formatPaise(p.mrr)}</p>
                <p style={{ fontSize: 10, color: colors.text.muted, marginTop: 2 }}>MRR</p>
              </div>
            </div>
          ))
        )}
              ...
              <thead>
                <tr style={{ background: colors.surface.bg, borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
                  {['Plan', 'Price / mo', 'Max Jobs', 'Max Seats', 'Add-ons'].map(h => (
                    <th key={h} style={{ textAlign: 'left', padding: '10px 20px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: colors.text.muted }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {plans.map((p, idx) => (
                  ...
                  >
                    <td style={{ padding: '12px 20px', fontWeight: 600, color: colors.text.ink, textTransform: 'capitalize' }}>{p.name}</td>
                    <td style={{ padding: '12px 20px', color: colors.text.ink, fontVariantNumeric: 'tabular-nums' }}>{formatPaise(p.price_monthly)}</td>
                    <td style={{ padding: '12px 20px', color: colors.text.muted, fontVariantNumeric: 'tabular-nums' }}>{p.max_active_jobs ?? '∞'}</td>
                    <td style={{ padding: '12px 20px', color: colors.text.muted, fontVariantNumeric: 'tabular-nums' }}>{p.max_recruiter_seats ?? '∞'}</td>
                    <td style={{ padding: '12px 20px' }}>
                      <div className="flex flex-wrap gap-1">
                        {p.resume_access && <span style={{ fontSize: 10, padding: '2px 6px', background: colors.surface.elevated, color: colors.text.ink, borderRadius: 4 }}>Resume Access</span>}
                        {p.candidate_search_limit !== null && <span style={{ fontSize: 10, padding: '2px 6px', background: colors.surface.bg, color: colors.text.muted, borderRadius: 4 }}>Search: {p.candidate_search_limit}/mo</span>}
