...
export default function EmployerReportsPage() {
  ...
  return (
    <section className="flex flex-col gap-6">
      ...
      <div>
        <h1 style={{ fontSize: 20, fontWeight: 800, color: colors.text.ink, fontFamily: 'Hind, sans-serif', letterSpacing: '-0.3px' }}>Employer Reports</h1>
        <p className="text-sm mt-1" style={{ color: colors.text.muted }}>Employer growth, verification trends, and hiring engagement.</p>
      </div>
        ...
        <>
          ...
          <div style={cardStyle}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1.5px', color: colors.text.muted, whiteSpace: 'nowrap' }}>Employer Engagement Rate</span>
              <div style={{ flex: 1, height: '0.5px', background: '#E2E8F0' }} />
            </div>
            <p style={{ fontSize: 36, fontWeight: 800, color: colors.text.ink, marginBottom: 4 }}>{engagementRate}%</p>
            <p className="text-xs" style={{ color: colors.text.muted }}>of registered employers have at least one job posted</p>
            <div className="mt-4 h-2.5 rounded-full overflow-hidden" style={{ background: colors.surface.elevated }}>
              <div className="h-full rounded-full transition-all" style={{ width: `${engagementRate}%`, background: colors.brand.navy }} />
            </div>
          </div>
              ...
              <div className="p-5 flex flex-col gap-3">
                {[
                  ...
                  <div key={label} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: colors.surface.elevated }}>
                      <div className="h-full rounded-full" style={{ width: `${(count / employers.length) * 100}%`, background: color }} />
                    </div>
                    <p className="text-xs font-bold w-8 text-right tabular-nums" style={{ color: colors.text.ink }}>{count}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {cohortRows.length > 0 && (
            <div style={cardStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1.5px', color: colors.text.muted, whiteSpace: 'nowrap' }}>Platform User Growth (last 6 months)</span>
                <div style={{ flex: 1, height: '0.5px', background: '#E2E8F0' }} />
              </div>
                ...
                {cohortRows.map(row => (
                  <div key={row.month} className="flex-1 flex flex-col items-center gap-1.5">
                    <div
                      ...
                      style={{ height: `${(row.signups / maxGrowth) * 100}%`, minHeight: 3, width: '100%', background: colors.brand.navy, borderRadius: 2, opacity: 0.8, transition: 'opacity 0.15s' }}
                      onMouseOver={e => (e.currentTarget.style.opacity = '1')}
                      onMouseOut={e => (e.currentTarget.style.opacity = '0.8')}
                    />
                    <span className="text-[10px]" style={{ color: colors.text.muted }}>{row.month.slice(5)}</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                <thead>
                  <tr style={{ background: colors.surface.bg, borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
                    {['Company', 'Jobs', 'Status'].map((h, i) => (
                      <th key={h} className={i > 0 ? 'text-right' : 'text-left'} style={{ padding: '10px 20px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: colors.text.muted }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[...employers]
                    ...
                    .slice(0, 10)
                    .map((e, idx) => (
                      <tr key={e.id} style={{ background: idx % 2 === 0 ? '#fff' : colors.surface.bg, borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                        <td className="truncate max-w-[240px] font-medium" style={{ padding: '12px 20px', color: colors.text.ink }}>{e.company_name}</td>
                        <td className="text-right font-bold tabular-nums" style={{ padding: '12px 20px', color: colors.text.ink }}>{e.job_count ?? 0}</td>
