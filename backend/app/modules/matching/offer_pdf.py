...
def generate_offer_letter_pdf(
    *,
    ...
    """Return raw PDF bytes for an offer letter.

    When `signed` is True, the candidate-signature line is replaced with the
    typed name + acceptance timestamp/IP audit trail instead of a blank line —
    this is a "click to accept" e-signature, not a cryptographic/DocuSign-grade
    one (see docs/ENTERPRISE_AUDIT_ROADMAP.md M2 for why).
    """
    try:
    ...
    sig_data = [
        ...
        [
            ...
            Paragraph(signature_name if signed and signature_name else candidate_name, SIG_L),
        ],
        [
            Paragraph(hiring_manager_designation, SIG_S),
            Paragraph(f"Signed: {signed_at}" if signed and signed_at else "Date: ___________________", SIG_S),
        ],
    ]
    ...
    if signed and signature_name:
        audit_bits = [f"digitally accepted by <b>{signature_name}</b>"]
        if signed_at:
            audit_bits.append(f"on {signed_at}")
        if signature_ip:
            audit_bits.append(f"from IP {signature_ip}")
        story.append(Paragraph(
            "Candidate acceptance record: " + " ".join(audit_bits) + ".",
            sty("Audit", fontName="Helvetica-Oblique", fontSize=7.5, textColor=NAVY, alignment=TA_CENTER, spaceAfter=3),
        ))
